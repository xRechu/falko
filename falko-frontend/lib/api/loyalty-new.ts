...
export async function fetchCustomerLoyaltyPoints(): Promise<LoyaltyPointsResponse> {
  try {
    ...
    if (!response.ok) {
      console.warn(`⚠️ Loyalty API failed (${response.status}), using mock data`);
      return fallbackToMockLoyaltyData();
    }

    const data = await response.json();
    console.log('✅ Successfully fetched loyalty points:', data);
    return data;
    
  } catch (error) {
    console.warn('⚠️ Error fetching loyalty points, using mock data:', error);
    return fallbackToMockLoyaltyData();
  }
}
...
export async function fetchLoyaltyRewards(): Promise<LoyaltyRewardResponse[]> {
  try {
    ...
    const isBackendHealthy = await checkBackendHealth();
    if (!isBackendHealthy) {
      console.warn('⚠️ Backend unavailable, using mock rewards');
    ...
    console.warn('⚠️ Error fetching loyalty rewards, using mock data:', error);
    return fallbackToMockRewards();
  }
}
...
export async function redeemLoyaltyReward(rewardId: string): Promise<RedeemRewardResponse> {
  try {
    ...
    const data = await response.json();
    console.log('✅ Successfully redeemed reward:', data);
    return data;
